    ...
    <div className="group relative bg-white rounded-2xl overflow-hidden border border-slate-100 hover:shadow-2xl hover:shadow-slate-200 transition-all duration-300 hover:-translate-y-1">
      ...
      <div className="aspect-[3/4] overflow-hidden relative bg-slate-100">
        ...
        <div className="absolute inset-x-0 bottom-0 p-3 sm:p-4 translate-y-0 sm:translate-y-full sm:group-hover:translate-y-0 transition-transform duration-300">
          <button
            onClick={handleadd}
            className={`w-full ${isadded ? 'bg-pop-pink' : 'bg-white hover:bg-slate-50 active:scale-95'} text-slate-900 font-bold py-2.5 sm:py-3 rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all`}
          >
      ...
      <div className="p-3 sm:p-5">
        ...
        <div className="text-xs md:text-sm text-slate-500 mb-2 flex flex-col md:flex-row md:items-center">
          {product.brand && (
        ...
        <div className="flex flex-wrap gap-1 mt-2 sm:mt-3">
          {product.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className="text-[9px] sm:text-[10px] font-bold text-slate-400 bg-slate-50 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md uppercase tracking-wide"
            >
              #{tag}
            </span>
          ))}
